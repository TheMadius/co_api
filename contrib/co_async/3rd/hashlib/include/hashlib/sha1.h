...
public:
  ...
  inline std::string operator()(const void* data, size_t numBytes);
  /// compute SHA1 of a string, excluding final zero
  inline std::string operator()(const std::string& text);

  /// add arbitrary number of bytes
  inline void add(const void* data, size_t numBytes);

  /// return latest hash as 40 hex characters
  inline std::string getHash();
  /// return latest hash as bytes
  inline void        getHash(unsigned char buffer[HashBytes]);
...
private:
  /// process 64 bytes
  inline void processBlock(const void* data);
